...
{
    public class DummySequence3 : SequentialTestSequence<StationConfig, ProductConfig, TestConfig>
    {
        protected override void ExecuteTest(CancellationToken token, StationConfig stationConfig, ProductConfig productConfig, TestConfig testConfig)
        {
            try
            {
                var c = TestClient.Instruments.DummyPowerSupply.Connect();
                throw new ResponseException(c.Connection);
            }
