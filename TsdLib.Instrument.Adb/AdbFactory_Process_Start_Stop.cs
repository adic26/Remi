...
namespace TsdLib.Instrument.Adb
{
    ...
    /// </summary>
    public class AdbFactory_Process_Start_Stop : FactoryBase<AdbConnection_Process_StartStop>
    {
        ...
        protected override AdbConnection_Process_StartStop CreateConnection(string address, int defaultDelay, params ConnectionSettingAttribute[] attributes)
        {
            ...
            AdbConnection_Process_StartStop conn = new AdbConnection_Process_StartStop(address, AdbFileLocation);

            return conn;
        }

        protected override string GetInstrumentIdentifier(AdbConnection_Process_StartStop connection, IdQueryAttribute idAttribute)
        {
            return "Avengers";
        }
    }
